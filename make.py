...
def build():
  ...
  args = '-O3 --llvm-lto 1 -s NO_EXIT_RUNTIME=1 -s NO_FILESYSTEM=1 -s EXPORTED_RUNTIME_METHODS=[] -s MODULARIZE_INSTANCE=1 -s EXPORT_ES6=1 '
  if not wasm:
    ...
    args += ''' -s WASM=1 -s BINARYEN_IGNORE_IMPLICIT_TRAPS=1 -s BINARYEN_TRAP_MODE="clamp" -s ASSERTIONS=1 '''
  if closure:
